...
$soyad=$_POST['soyad'];	
$tel=$_POST['tel'];
$kiralama=$_POST['kiralama'];
$birakma=$_POST['birakma'];
$aciklama=$_POST["aciklama"];	
$durum="0";				
							
if(empty($ad) || empty($soyad) || empty($tel) || empty($kiralama)|| empty($birakma)|| empty($aciklama)){
echo "Tüm alanları doldurunuz";
}
                $sorgu="insert into rezervasyon set ad='".$ad."', soyad='".$soyad."', telefon='".$tel."',kiralama='".$kiralama."',birakma='".$birakma."',aciklama='".$aciklama."',durum='".$durum."'";             
                $ekle=mysql_query($sorgu);
               
                if($ekle) {
                               echo "Rezervasyon işleminiz başarıyla gerçekleştirilmiştir. Admin tarafından onaya sunulmuştur.5 saniye içinde Ana sayfaya yönlendiriliyorsunuz";
							   header( "refresh:5; url=index2.php" );
                }else{
                               echo "Rezerve İşlemi Yapılamadı. O Tarihler rezerve edilmiş..!";
                }
