...
$aciklama=$_POST['aciklama'];
	...
	if($_POST){
			if ($_FILES["gelinlik_resim"]["size"]<2048*2048){
				if ($_FILES["gelinlik_resim"]["type"]=="image/jpeg" or "image/jpg" or "image/png" or "image/gif"){
					$dosya_adi=$_FILES["gelinlik_resim"]["name"];
					$uret=array("as","rt","ty","yu","fg");
					$uzanti=substr($dosya_adi,-4,4);
					$sayi_tut=rand(1,10000);
					$yeni_ad="dosyalar/".$uret[rand(0,4)].$sayi_tut.$uzanti;
					if (move_uploaded_file($_FILES["gelinlik_resim"]["tmp_name"],$yeni_ad)){
...
echo "Tüm alanları doldurunuz";
                ...
                $sorgu="insert into gelinlik_ekle set model='".$model."', beden='".$beden."', fiyat='".$fiyat."',aciklama='".$aciklama."',gelinlik_resim='".$yeni_ad."'";             
                $ekle=mysql_query($sorgu);
               
                if($ekle) {
                               echo "Gelinlik başarı ile eklendi.Anasayfaya yönlendiriliyorsunuz:";
							   header("Location:admin.php");
                }else{
                               echo "Gelinlik eklenemedi";
                }
	}
